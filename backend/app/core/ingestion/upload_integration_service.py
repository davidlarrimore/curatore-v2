...
class UploadIntegrationService:
    ...
    async def create_asset_from_upload(
        ...
        """
        # Build namespaced source metadata with provenance
        source_metadata = {
            "source": {
                "storage_folder": "/".join(artifact.object_key.split("/")[1:-1]) if "/" in artifact.object_key else "",
        ...
        # Remove None values within source namespace
        source_metadata["source"] = {
            k: v for k, v in source_metadata["source"].items() if v is not None
        }
        # Merge additional metadata at top level (may include custom namespaces)
    ...
    async def link_asset_to_document_id(
        ...
        return await asset_service.update_asset_metadata(
            session=session,
            asset_id=asset_id,
            source_metadata={"source": {"document_id": document_id}},
        )
