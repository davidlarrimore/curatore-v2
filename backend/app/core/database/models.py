...
class Run(Base):
    ...
    trace_id = Column(String(36), nullable=True, index=True)  # Root trace UUID — groups related runs
    parent_run_id = Column(UUID(), ForeignKey("runs.id", ondelete="SET NULL"), nullable=True, index=True)
...
class AssetMetadata(Base):
    """
    AssetMetadata model for flexible, versioned document metadata.

    Stores derived metadata as first-class artifacts, enabling LLM-driven iteration
    without schema churn. Each asset can have multiple metadata types (topics,
    summary, tags, etc.) with a single canonical version per type.
    ...
    Attributes:
        ...
        is_canonical: Whether this is the canonical metadata for its type (default True)
    ...
    Usage:
        metadata = AssetMetadata(
            ...
            metadata_content={"tags": ["contract", "federal"]},
        )
    """
    ...
    # Canonical flag (default True — simplified from promotion ceremony)
    is_canonical = Column(Boolean, nullable=False, default=True, index=True)
...
class MetadataFieldDefinition(Base):
    """
    Registry of metadata field definitions per namespace.

    Global baseline fields have organization_id=NULL. Org-level overrides
    have a specific organization_id and merge with the global baseline
    at runtime via the MetadataRegistryService.
    """
    ...
    data_type = Column(String(50), nullable=False)  # string, number, boolean, date, enum, array, object
    indexed = Column(Boolean, nullable=False, default=True)
    facetable = Column(Boolean, nullable=False, default=False)
    applicable_content_types = Column(JSONB, nullable=False, default=list, server_default="[]")
    description = Column(Text, nullable=True)
    examples = Column(JSONB, nullable=True)
    sensitivity_tag = Column(String(50), nullable=True)
    ...
    # Relationships
    organization = relationship("Organization", backref="metadata_field_definitions")

    __table_args__ = (
        Index("ix_field_defs_org_namespace", "organization_id", "namespace"),
        Index("ix_field_defs_status", "status"),
    )

    def __repr__(self) -> str:
        return f"<MetadataFieldDefinition({self.namespace}.{self.field_name}, org={self.organization_id})>"


class FacetDefinition(Base):
    """
    Cross-domain facet definitions.

    Facets are user-facing filter abstractions that map to different
    JSON paths in different content types (e.g., 'agency' maps to
    sam.agency for SAM notices and forecast.agency_name for forecasts).
    """
    ...
    data_type = Column(String(50), nullable=False)  # string, number, boolean, date
    description = Column(Text, nullable=True)
    operators = Column(JSONB, nullable=True, default=lambda: ["eq", "in"])
    status = Column(String(20), nullable=False, default="active")
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)

    # Relationships
    organization = relationship("Organization", backref="facet_definitions")
    mappings = relationship("FacetMapping", backref="facet_definition", cascade="all, delete-orphan")

    __table_args__ = (
        Index("ix_facet_defs_org", "organization_id"),
    )

    def __repr__(self) -> str:
        return f"<FacetDefinition({self.facet_name}, org={self.organization_id})>"


class FacetMapping(Base):
    """
    Maps a facet to a specific JSON path within a content type.

    For example, the 'agency' facet maps to:
