...
from app.database.models import Asset, SharePointSyncedDocument
...
async def find_duplicate_sharepoint_assets(
    ...
    if sync_config_id:
        query = query.where(
            Asset.source_metadata["sync_config_id"].astext == str(sync_config_id)
        )
    ...
    for asset in assets:
        item_id = asset.source_metadata.get("sharepoint_item_id")
        if item_id:
