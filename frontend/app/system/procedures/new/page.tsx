...
export default function SystemNewProcedurePage() {
  ...
  const handleValidate = useCallback(async () => {
    ...
    try {
      const result = await systemCwrApi.validateProcedure(procedure)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (savedProcedure) {
        ...
        saved = await systemCwrApi.updateProcedure(savedProcedure.slug, {
          name: procedure.name,
        ...
        saved = await systemCwrApi.createProcedure(procedure)
      }
      ...
      if (!savedProcedure) {
        router.replace(`/system/procedures/${saved.slug}/edit`)
      }
    } catch (err: unknown) {
      const error = err as { validation?: { errors?: ValidationError[]; warnings?: ValidationError[] }; message?: string }
      if (error.validation) {
        setValidationErrors(error.validation.errors || [])
        setValidationWarnings(error.validation.warnings || [])
        ...
        setErrorMessage(error.message || 'Failed to save procedure')
      }
      setTimeout(() => setErrorMessage(''), 5000)
    } finally {
      setIsSaving(false)
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                  {savedProcedure
                    ? savedProcedure.name
                    : parsedInfo.name || 'New System Procedure'}
                </h1>
                          ...
                          {warning.details && Object.keys(warning.details).length > 0 && (
                            <details className="mt-2">
                              ...
                              <div className="mt-1 text-xs text-gray-600 dark:text-gray-400 bg-amber-50 dark:bg-amber-900/20 p-2 rounded">
                                {(warning.details as Record<string, unknown>).suggestion as string || JSON.stringify(warning.details, null, 2)}
                              </div>
                            </details>
                          )}
                        </div>
                                    ...
                                    <div className="mt-2 ml-5 space-y-2">
                                      ...
                                      {func.tags && func.tags.length > 0 && (
                                        <div className="flex flex-wrap gap-1">
                                          {func.tags.map((tag) => (
                                            <span key={tag} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400">
                                        ...
                                        <div>
                                          ...
                                          {func.output_schema.properties && Object.keys(func.output_schema.properties).length > 0 && (
                                            <div className="mt-1 space-y-0.5">
                                              {Object.entries(func.output_schema.properties).map(([name, prop]) => {
                                                ...
                                                return (
                                                  <div key={name} className="flex items-start gap-1.5 text-[10px]">
                                                    ...
                                                    <span className="text-gray-400">({propObj.type as string})</span>
                                                    <span className="text-gray-500 truncate">{propObj.description as string}</span>
                                                  </div>
                                                )
                                              })}
                                            </div>
                                          ...
                                          {func.output_schema.items?.properties && Object.keys(func.output_schema.items.properties).length > 0 && (
                                            <div className="mt-1 space-y-0.5">
                                              {Object.entries(func.output_schema.items.properties).map(([name, prop]) => {
                                                const propObj = prop as Record<string, unknown>
                                                return (
                                                  <div key={name} className="flex items-start gap-1.5 text-[10px]">
                                                    <span className="font-mono text-gray-600 dark:text-gray-400">.{name}</span>
                                              ...
                                              {func.output_schema.variants.map((variant, idx) => {
                                                ...
                                                return (
                                                  <div key={idx} className="text-[10px] text-gray-500 dark:text-gray-400 italic">
                                                    {variantObj.description as string}
                                                  </div>
                                                )
                                              })}
                                            </div>
                                      ...
                                      {func.examples && func.examples.length > 0 && (
                                        <details className="group">
                                          <summary className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-amber-600 dark:hover:text-amber-400">
...
{`function: ${func.name}
params:
${Object.entries((exObj.params || {}) as Record<string, unknown>).map(([k, v]) =>
