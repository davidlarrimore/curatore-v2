...
import {
  ...
  type FacetDiscoverResult,
  type FacetPendingSuggestions,
...
export default function SystemMetadataCatalogPage() {
  ...
  const [discoverResult, setDiscoverResult] = useState<FacetDiscoverResult | null>(null)
  ...
  // Load pending suggestion counts when entering reference tab
  const loadPendingSuggestions = useCallback(async () => {
    try {
      const counts = await systemMetadataApi.getPendingSuggestionCount()
      setPendingSuggestions(counts)
    } catch {
  ...
  useEffect(() => {
    if (activeTab === 'reference') loadPendingSuggestions()
  ...
  const handleDiscover = async () => {
    if (!selectedFacetForRef) return
    setIsDiscovering(true)
    setDiscoverResult(null)
    try {
      const result = await systemMetadataApi.discoverReferenceValues(selectedFacetForRef)
      setDiscoverResult(result)
      if (result.unmapped_count === 0) {
        toast.success('No unmapped values found — all indexed values are already mapped')
      } else {
        toast.success(`Found ${result.unmapped_count} unmapped value(s)`)
      }
      // Reload reference values to pick up any newly-created suggestions
      loadReferenceValues(selectedFacetForRef)
  ...
  const handleApproveRefValue = async (valueId: string) => {
    try {
      await systemMetadataApi.approveReferenceValue(selectedFacetForRef, valueId)
      toast.success('Reference value approved')
      loadReferenceValues(selectedFacetForRef)
      loadPendingSuggestions()
    } catch (err: unknown) {
  ...
  const handleRejectRefValue = async (valueId: string) => {
    try {
      await systemMetadataApi.rejectReferenceValue(selectedFacetForRef, valueId)
      toast.success('Reference value rejected')
      loadReferenceValues(selectedFacetForRef)
      loadPendingSuggestions()
    } catch (err: unknown) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {([
            ...
            { key: 'reference' as ActiveTab, label: 'Reference Data', icon: BookOpen, count: pendingSuggestions?.total || undefined },
        ...
        {activeTab === 'reference' && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  onChange={(e) => { setSelectedFacetForRef(e.target.value); setDiscoverResult(null) }}
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  disabled={isDiscovering || !selectedFacetForRef}
                  className="flex items-center gap-1.5 text-sm text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 disabled:opacity-50"
                >
            ...
            {discoverResult && discoverResult.unmapped_count > 0 && (
              <div className="px-6 py-4 bg-amber-50 dark:bg-amber-900/10 border-b border-amber-200 dark:border-amber-800/30">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                    <span className="text-sm font-medium text-amber-800 dark:text-amber-300">
                      {discoverResult.unmapped_count} unmapped value(s) found in indexed documents
                    </span>
                  </div>
                  <button
                ...
                <div className="space-y-1.5">
                  {discoverResult.unmapped_values.map((uv) => (
                    <div key={uv.value} className="flex items-center gap-2 text-xs">
                      <span className="font-medium text-amber-800 dark:text-amber-300">{uv.value}</span>
                      <span className="text-amber-500 dark:text-amber-500 text-[10px]">({uv.count} docs)</span>
                      {uv.sources && uv.sources.length > 0 && (
                        <div className="flex gap-1">
                          {uv.sources.map((src) => (
                            <span key={src} className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-200/60 dark:bg-amber-800/40 text-amber-700 dark:text-amber-400">
                              {src}
                            </span>
                          ))}
                        </div>
                ...
                {discoverResult.suggestions && discoverResult.suggestions.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-amber-200/50 dark:border-amber-800/20">
                    <p className="text-xs font-medium text-amber-700 dark:text-amber-400 mb-2">Suggestions (auto-generated):</p>
                    <div className="space-y-1">
                      {discoverResult.suggestions.map((s, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs text-amber-800 dark:text-amber-300">
                          <span className="font-medium">{s.unmapped_value || s.value}</span>
                          <span className="text-amber-500">→</span>
                          <span>{s.suggested_canonical || s.canonical_value || 'New value'}</span>
                          {s.method && <span className="text-amber-500 text-[10px]">[{s.method}]</span>}
                          {s.confidence != null && <span className="text-amber-500 text-[10px]">({Math.round(s.confidence * 100)}%)</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {discoverResult.error && (
                  <p className="mt-2 text-xs text-red-600 dark:text-red-400">{discoverResult.error}</p>
                )}
              </div>
            )}
                ...
                {referenceValues.map((rv) => (
                  <div key={rv.id}>
                    <div
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleValue(rv.id) } }}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {rv.status === 'suggested' && (
                          <>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleApproveRefValue(rv.id) }}
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); handleRejectRefValue(rv.id) }}
                              className="p-1 text-gray-400 hover:text-red-500 transition-colors"
                              title="Reject"
                            >
