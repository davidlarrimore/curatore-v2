...
/**
 * System-wide job manager page.
 * Shows jobs across all organizations for system administrators.
 */
...
export default function SystemJobManagerPage() {
  ...
  // Compute stats for queue tabs (active jobs only)
  ...
  const handleForceKill = async (runId: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to force kill job'
      setError(message)
      setTimeout(() => setError(''), 5000)
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Monitor and manage all background jobs across all organizations
                </p>
              </div>
            </div>
            ...
            {JOB_TYPE_TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {tab.label}
                  {count > 0 && (
                    <span className={`px-1.5 py-0.5 rounded-full text-xs ${isActive ? 'bg-amber-200 dark:bg-amber-800' : 'bg-gray-100 dark:bg-gray-700'}`}>
                      {count}
                    </span>
                  )}
                </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          <button
            ...
            className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-4 transition-all ${statusFilter === 'pending' ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300'}`}
          >
            <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-4 transition-all ${statusFilter === 'running' ? 'border-purple-500 ring-2 ring-purple-500/20' : 'border-gray-200 dark:border-gray-700 hover:border-purple-300'}`}
          >
            <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-4 transition-all ${statusFilter === 'completed' ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-gray-200 dark:border-gray-700 hover:border-emerald-300'}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-xs text-gray-500 dark:text-gray-400">Done (24h)</p>
          ...
          <button
            ...
            className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-4 transition-all ${statusFilter === 'failed' ? 'border-red-500 ring-2 ring-red-500/20' : 'border-gray-200 dark:border-gray-700 hover:border-red-300'}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>

                {selectedIds.size > 0 && cancellableSelectedIds.length > 0 && (
                  <Button
                    variant="secondary"
                  ...
                  >
                    {isBulkCancelling ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                ...
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                    <th className="px-4 py-3 w-10">
                      <button onClick={toggleSelectAll} className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredJobs.map((job) => {
                    ...
                    return (
                      <tr
                        key={job.run_id}
                        onClick={() => router.push(`/system/jobs/${job.run_id}`)}
                        className={`hover:bg-gray-50 dark:hover:bg-gray-900/30 cursor-pointer ${selectedIds.has(job.run_id) ? 'bg-amber-50 dark:bg-amber-900/10' : ''}`}
                      >
                        <td className="px-4 py-4 w-10" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => toggleSelect(job.run_id)} className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700">
                        ...
                        <td className="px-4 py-4">
                          <div className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-lg text-xs font-medium bg-${color}-50 text-${color}-700 dark:bg-${color}-900/20 dark:text-${color}-400`}>
                        ...
                        <td className="px-4 py-4">
                          <div className="min-w-0">
                            ...
                            {job.display_context && (
                              <p className="text-xs text-gray-500 dark:text-gray-400 font-mono">{job.display_context}</p>
                            )}
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {job.can_cancel && (
                              <button
                                ...
                                title="Cancel job"
                              >
                                {cancellingIds.has(job.run_id) ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4" />}
                              </button>
                            )}
                            {['pending', 'submitted', 'running', 'stale'].includes(job.status) && (
                              <button
                                ...
                                className="p-1.5 rounded-lg text-orange-600 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 disabled:opacity-50"
                                title="Force kill"
                              >
                                {forceKillingIds.has(job.run_id) ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
                              </button>
                            )}
                          </div>
                        </td>
