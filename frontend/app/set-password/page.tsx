 ...
 *
 * Uses the same reset token mechanism as password reset,
 * but with invitation-specific messaging.
 */
...
function SetPasswordContent() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!token) {
      setError('Invalid or missing invitation token. Please contact your administrator.')
      return
    }
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Choose a password to activate your Curatore account
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <Button type="submit" disabled={isLoading || !token} className="w-full">
            {isLoading ? 'Setting up...' : 'Set Password & Activate Account'}
          </Button>
        </form>
      </div>
    </div>
