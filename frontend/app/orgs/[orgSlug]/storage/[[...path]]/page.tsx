...
/**
 * Organization-scoped storage browser page.
 */
...
import { objectStorageApi, assetsApi, utils } from '@/lib/api'
import { formatDateTime } from '@/lib/date-utils'
import StorageFolderBrowser from '@/components/storage/StorageFolderBrowser'

// Lazy load FilePreview - heavy component with PDF, Word, Excel parsing libraries
const FilePreview = dynamic(
  () => import('@/components/storage/FilePreview'),
  {
    loading: () => (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <div className="w-8 h-8 rounded-full border-4 border-gray-200 dark:border-gray-700 border-t-indigo-500 animate-spin mx-auto" />
          <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">Loading preview...</p>
        </div>
      </div>
    ),
    ssr: false,
...
export default function StoragePage() {
  ...
  // Translate slug to UUID for S3 API calls (uses organization from URL context)
  const urlPrefix = organization && rawPrefix.startsWith(organization.slug + '/')
    ? organization.id + '/' + rawPrefix.slice(organization.slug.length + 1)
    : rawPrefix
  ...
  useEffect(() => {
    if (bucketsLoaded && !urlBucket && buckets.length > 0) {
      ...
      router.replace(orgUrl(`/storage/${defaultBucket.name}`))
    }
  }, [bucketsLoaded, urlBucket, buckets, router, orgSlug])

  // Navigate handler: updates URL when user clicks folders/breadcrumbs/bucket selector
  // Translates org UUID to slug for human-readable URLs
  const handleNavigate = (bucket: string, prefix: string) => {
    const displayPrefix = organization && prefix.startsWith(organization.id + '/')
      ? organization.slug + '/' + prefix.slice(organization.id.length + 1)
      : prefix
    const prefixPath = displayPrefix ? '/' + displayPrefix.replace(/\/$/, '') : ''
    router.push(orgUrl(`/storage/${bucket}${prefixPath}`))
  }
  ...
  const loadHealthData = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load storage health'
      setError(message)
    } finally {
      setIsLoading(false)
    }
