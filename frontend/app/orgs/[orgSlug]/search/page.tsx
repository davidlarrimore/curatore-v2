...
/**
 * Organization-scoped search page.
 * Hybrid full-text + semantic search across all document sources.
 */
...
export default function SearchPage() {
  ...
  const executeSearch = useCallback(async () => {
    ...
    try {
      const activeFacetFilters: Record<string, string | string[]> = {}
      ...
      if (httpErr?.status === 503) {
        ...
        setError(httpErr?.message || 'Search failed')
      }
      setResults(null)
      setFacets(null)
    } finally {
  ...
  // Navigate to detail page based on source type
  const handleResultClick = (hit: SearchHit) => {
    if (hit.source_type === 'Account') {
      router.push(orgUrl(`/syncs/salesforce/accounts/${hit.asset_id}`))
    } else if (hit.source_type === 'Contact') {
      router.push(orgUrl(`/syncs/salesforce/contacts/${hit.asset_id}`))
    } else if (hit.source_type === 'Opportunity') {
      router.push(orgUrl(`/syncs/salesforce/opportunities/${hit.asset_id}`))
    } else if (hit.source_type === 'AG Forecast' || hit.source_type === 'APFS Forecast' || hit.source_type === 'State Forecast') {
      router.push(orgUrl(`/syncs/forecasts/${hit.asset_id}`))
    } else {
      router.push(orgUrl(`/assets/${hit.asset_id}`))
    }
  }
