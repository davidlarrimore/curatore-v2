...
export default function SharePointSyncPage() {
  ...
  const loadConfigs = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      setError(error.message || 'Failed to load sync configurations')
    } finally {
  ...
  const handleSync = async (configId: string, configName: string) => {
    ...
    try {
      ...
      const error = err as { message?: string }
      addToast('error', `Failed to start sync: ${error.message}`, configId)
    }
  }
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href={orgUrl('/syncs/sharepoint/new')}>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-3">
                          <Link
                            href={orgUrl(`/syncs/sharepoint/${config.id}`)}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {config.status !== 'deleting' && !isDeleting(config.id) && (
                        <Link href={orgUrl(`/syncs/sharepoint/${config.id}`)}>
