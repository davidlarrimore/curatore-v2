...
export default function NewSharePointSyncPage() {
  ...
  const handleBrowseFolder = async () => {
    ...
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to browse SharePoint folder')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleCreateSync = async () => {
    ...
    try {
      ...
      if (syncConfigId) {
        router.push(orgUrl(`/syncs/sharepoint/${syncConfigId}`))
      } else {
        router.push(orgUrl('/syncs/sharepoint'))
      }
    } catch (err: unknown) {
      const error = err as { message?: string }
      setError(error.message || 'Failed to create sync configuration')
