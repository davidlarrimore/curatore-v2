...
export default function SamSetupPage() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setError(error.message || 'Failed to load SAM data')
    } finally {
      if (!silent) {
        setIsLoading(false)
      }
  ...
  const handleTriggerPull = async (search: SamSearch) => {
    ...
    try {
      ...
      if (isRateLimit) {
        ...
        toast(error.message || 'Please wait before triggering another pull.', {
          icon: '⏳',
          duration: 4000,
        })
      } else {
        // Show error for actual failures
        setError(error.message || 'Failed to trigger pull')
        toast.error(error.message || 'Failed to trigger pull')
      }
    }
  }
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {sortedSearches.map((search) => (
                    <SearchTableRow
                      ...
                      onClick={() => router.push(orgUrl(`/syncs/sam/${search.id}`))}
