...
export default function SamSearchDetailPage({ params }: PageProps) {
  ...
  const loadSearch = useCallback(async () => {
    ...
    try {
      ...
      setError(error.message || 'Failed to load search')
    }
  }, [token, searchId])
  ...
  const loadPullHistory = useCallback(async () => {
    ...
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to load pull history')
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-6 text-center">
          <Link
            href={orgUrl('/syncs/sam/solicitations')}
