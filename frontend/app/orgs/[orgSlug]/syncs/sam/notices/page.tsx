  ...
  const loadNotices = useCallback(async () => {
    ...
    try {
      ...
      if (searchMode === 'semantic' && debouncedSearchQuery.trim()) {
        ...
        const noticeMap = new Map(data.items.map((n: SamNoticeWithSolicitation) => [n.id, n]))
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {paginatedNotices.map((notice) => (
                      <tr
                        key={notice.id}
                        onClick={() => router.push(orgUrl(`/syncs/sam/notices/${notice.id}`))}
                        ...
                        <td className="px-4 py-3">
                          {notice.solicitation_number ? (
                            notice.solicitation_id ? (
                              <Link
                                href={orgUrl(`/syncs/sam/solicitations/${notice.solicitation_id}`)}
