...
export default function SamNoticeDetailPage({ params }: PageProps) {
  ...
  const handleRefreshFromSam = async () => {
    ...
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to refresh from SAM.gov')
    } finally {
  ...
  const handleDownloadAttachments = async () => {
    ...
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to download attachments')
    } finally {
      ...
      <div>
        ...
        {!notice.is_standalone && solicitation && (
          <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <Link
              href={orgUrl(`/syncs/sam/solicitations/${solicitation.id}`)}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <SamBreadcrumbs
          items={[
            { label: 'Notices', href: orgUrl('/syncs/sam/notices') },
