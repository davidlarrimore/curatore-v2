 ...
 * View namespaces, fields, facets, and reference data for metadata governance.
 * Shows provenance badges (Master vs Custom) on each field/facet.
 */
...
type ActiveTab = 'namespaces' | 'facets' | 'reference'
...
export default function MetadataCatalogPage() {
  ...
  const loadReferenceValues = useCallback(async (facetName: string) => {
    if (!facetName || !token) return
    setIsLoadingRef(true)
    try {
      const values = await metadataApi.getReferenceValues(token, facetName)
  ...
  const filteredNamespaces = catalog?.namespaces.filter((ns) => {
    ...
    return ns.fields.some(f => f.field_name.toLowerCase().includes(query) || f.description?.toLowerCase().includes(query))
  }) || []

  const filteredFacets = catalog?.facets.filter((f) => {
    if (!searchQuery.trim()) return true
    const query = searchQuery.toLowerCase()
    return f.facet_name.toLowerCase().includes(query) || f.display_name.toLowerCase().includes(query) || f.description?.toLowerCase().includes(query)
  }) || []
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Browse metadata namespaces, fields, facets, and reference data
                </p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{catalog.total_indexed_docs.toLocaleString()}</p>
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{catalog.namespaces.reduce((sum, ns) => sum + ns.fields.length, 0)}</p>
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {([
            { key: 'namespaces' as ActiveTab, label: 'Namespaces', icon: Layers, count: catalog?.namespaces.length },
        ...
        {activeTab === 'namespaces' && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{filteredNamespaces.length} namespace{filteredNamespaces.length !== 1 ? 's' : ''}</p>
            </div>
                  ...
                  <div key={ns.namespace}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-sm text-gray-500 dark:text-gray-400">{ns.doc_count.toLocaleString()} docs</span>
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">{ns.fields.length} fields</span>
                      </div>
                    </button>
        ...
        {activeTab === 'facets' && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Cross-domain filters for search queries</p>
            </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <ProvenanceBadge source={facet.source} />
                            </div>
                ...
                {referenceValues.map((rv) => (
                  <div key={rv.id}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs px-1.5 py-0.5 rounded ${rv.status === 'active' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'}`}>
                              {rv.status}
                            </span>
                          </div>
                    ...
                    {expandedValues.has(rv.id) && rv.aliases && (
                      <div className="px-6 pb-4 border-t border-gray-100 dark:border-gray-700/50">
                        <p className="mt-3 text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Aliases</p>
                        <div className="flex flex-wrap gap-2">
                          {rv.aliases.map((alias) => (
                            <div key={alias.id} className="flex items-center gap-1.5 text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
