...
function ProcedureEditor() {
  ...
  const handleValidate = useCallback(async () => {
    ...
    try {
      const result = await proceduresApi.validateProcedure(token, fullProc as Parameters<typeof proceduresApi.validateProcedure>[1])
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push(orgUrl('/admin/procedures'))
    } catch (err: unknown) {
  ...
  const handleConvertToSystem = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to convert procedure'
      setErrorMessage(message)
      setTimeout(() => setErrorMessage(''), 5000)
    } finally {
