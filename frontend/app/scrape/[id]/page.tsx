...
export default function ScrapeCollectionDetailPage({ params }: PageProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const errorMsg = err.message || 'Failed to delete collection'
  ...
  }, [collection?.status, collectionId, isDeleting, router])
  ...
  async function handleFilterAssets(filter: 'all' | 'page' | 'document') {
    if (!token) return
        ...
        {activeTab === 'assets' && (
          <div>
            ...
            <div className="flex items-center gap-2 mb-4">
              {(['all', 'page', 'document'] as const).map((filter) => (
                <button
                  key={filter}
