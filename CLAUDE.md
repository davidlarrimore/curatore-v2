...
### Key Files
- `backend/app/cwr/contracts/tool_contracts.py` — `ToolContract` dataclass + `ContractGenerator` (converts `FunctionMeta` to JSON Schema contracts)
...
- `frontend/lib/api.ts` — `contractsApi` client + `ToolContract` TypeScript interface; `FunctionMeta` includes governance fields
...
4. Contract is auto-generated by `ContractGenerator` from the `FunctionMeta`
