...
- `./scripts/dev-up.sh` or `make up`: start backend, worker, frontend, redis, and supporting services with Docker Compose.
...
## Testing Guidelines
- Backend tests use `pytest`; name tests `test_*.py` and keep them near the service they cover.
...
- When updating document service connection settings, verify alignment across:
  - `config.yml` / `config.yml.example` extraction engine entries
  - Connection sync and schema in `backend/app/core/auth/connection_service.py`
  - Engine resolution in `backend/app/core/shared/document_service.py`
  - Document service adapter in `backend/app/connectors/adapters/document_service_adapter.py`
